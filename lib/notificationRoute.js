...
// 일부 알림은 게시글 대상이 아니라 targetType/targetId/boardId 가 없을 수 있다 (예: 공지, 이벤트, 시스템 메시지 등)
// 이때는 /students 로 이동한다
...
export function resolveNotificationUrl({ targetType, targetId, boardId, toastId } = {}) {
  ...
  if (targetType === 'post' && targetId != null && boardId != null) {
    ...
    // 새 일정 알림 — 달력이 ?eventId= 를 읽어 그 달로 옮기고 그 일정을 펼친다
    url = `/calendar?eventId=${encodeURIComponent(targetId)}`;
  }
