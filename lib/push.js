...
// "이 기기에서 알림을 켠 적이 있다" 표시. 켜 뒀던 알림이 브라우저 쪽 사정(권한 회수·구독 만료)으로 꺼지면
// 앱을 열었을 때 곧바로 "알림이 꺼져 있어요" 로 다시 켜기를 권한다 — 직접 끈 기기(OPT_OUT)와 구별하는 근거 (2026-09-24)
const WAS_ON_KEY = 'pilsa:pushWasOn';

export function wasPushOnThisDevice() {
  try {
    return localStorage.getItem(WAS_ON_KEY) === '1';
  } catch {
    return false;
  }
}

function setPushWasOn(value) {
  try {
    if (value) localStorage.setItem(WAS_ON_KEY, '1');
    else localStorage.removeItem(WAS_ON_KEY);
  } catch {
    // 무시
  }
}
...
export async function disablePushOnThisDevice() {
  ...
  setPushWasOn(false); // 직접 껐으니 "꺼져 있어요" 안내 대상도 아니다
  return result;
}
