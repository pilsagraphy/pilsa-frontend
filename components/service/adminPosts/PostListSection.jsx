...
// 필터 · 검색 · 페이지네이션 · 선택 · 조치는 댓글 관리와 동작이 같아 훅에 모아 뒀다.
// 여기 남는 것은 게시글 표에 무엇을 어떻게 그리느냐뿐이다.
export default function PostListSection({ title = '게시글 관리' }) {
  const list = useAdminModerationList({
    store: useAdminPostStore,
    idKey: 'postId',
    // 조치 모달의 '대상 게시글' 칸에는 제목을 보여준다
    contentKey: 'title',
    reportTarget: REPORT_TARGET_POST,
  });
    ...
    <div className={listSectionClass}>
      ...
      <div className="mb-[5px] mt-[5px] flex flex-col gap-3 md:mb-4 md:mt-[10px] md:flex-row md:flex-wrap md:items-center md:justify-between">
        ...
        <div className="flex min-w-0 flex-1 basis-[260px] flex-row items-center gap-2">
          ...
          {/* 검색창은 120px 아래로는 줄지 않는다 — 그보다 좁으면 검색어가 두세 글자밖에 안 보인다.
              넓을 땐 296px 까지. 그래도 모자라면 버튼 묶음이 아랫줄 오른쪽으로 내려간다 (PM, 2026-09-26) */}
        ...
        {/* 폰에서도 한 줄 — 세로로 쌓으면 목록보다 버튼이 먼저 화면을 채운다. 아랫줄로 내려가면 오른쪽에 붙는다 */}
      ...
      <PostTable
        ...
        onBlind={(post) => list.openRowConfirm('blind', post)}
        onDelete={(post) => list.openRowConfirm('delete', post)}
