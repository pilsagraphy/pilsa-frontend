...
export default function InfoComments({ postId, comments = [], onChanged }) {
  ...
  const runUpdate = async (commentId, content, priv) => {
    try {
      await updateInfoComment(postId, commentId, { content, isPrivate: priv });
      resetInput();
      onChanged?.();
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    if (editingId) {
      ...
      setConfirmState({
        ...
        onConfirm: () => runUpdate(targetId, content, priv),
      });
      return;
    }
  ...
  return (
    <section className="flex w-full flex-col gap-8 md:gap-[60px]">
      ...
      <ReportModal
        ...
        targetContent={
          reportTarget
            ? `정보게시판 / ${reportTarget.isPrivate ? '비밀 댓글' : (reportTarget.content ?? '')}`
            : ''
        }
      />
