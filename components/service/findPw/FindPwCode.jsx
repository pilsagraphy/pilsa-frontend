...
export default function FindPwCode({ email, onNext, onPrev }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = emailCodeSchema.safeParse(normalizedCode);
    ...
    setLoading(true);
    try {
      const verified = await verifyEmailCode(email, normalizedCode);
      ...
      // 성공 메시지를 보여준 뒤 1초 후 4단계(비밀번호 재설정)로 이동
      timerRef.current = setTimeout(() => {
        onNext();
      }, 1000);
    } catch (err) {
      // 인증번호 불일치 혹은 만료 시 처리 토스트 메시지 표시
      toast.error('인증 실패', {
