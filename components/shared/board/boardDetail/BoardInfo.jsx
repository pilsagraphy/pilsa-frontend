...
export default function BoardInfo({ badgeLabel, title, date, author, stateLabel }) {
  ...
  return (
    <section className="w-full">
      ...
      <div className="flex min-h-0 items-center py-3 xl:h-[56px] xl:py-0">
        <div className="flex min-w-0 flex-wrap items-center gap-2 xl:gap-[12px]">
          {badgeLabel && <Badge label={badgeLabel} />}
          {/* 상태를 붙일 때만 flex-1 을 뺀다. flex-1 이면 제목이 남은 폭을 다 차지해
              상태가 오른쪽 끝으로 밀려 '제목 옆'이 아니게 된다.
              상태가 없는 사용자 상세는 지금까지와 똑같이 flex-1 을 유지한다. */}
          <h2
            className={`min-w-0 text-[16px] leading-snug tracking-[-0.36px] text-[#212121] xl:text-[18px] xl:leading-none ${
              stateLabel ? '' : 'flex-1'
            }`}
          >
            {safeTitle}
      ...
      <div className="flex flex-col gap-3 py-3 text-[13px] tracking-[-0.28px] xl:h-[56px] xl:flex-row xl:items-center xl:justify-between xl:gap-0 xl:py-0 xl:text-[14px]">
        <div className="flex flex-wrap items-center gap-2 xl:gap-[12px]">
          <span className="shrink-0 text-[#919191] leading-none">등록일</span>
          <VLine />
