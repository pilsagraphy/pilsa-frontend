...
// 같은 VAPID 키로 다시 구독한 뒤, 옛 endpoint 를 열쇠 삼아 서버 행을 새 구독으로 바꾼다(PUT /api/auth/push/rotate, 인증 없음).
// 예전엔 재구독만 하고 서버 등록은 다음 앱 실행·로그인 때로 미뤘는데, 그 사이 알림이 끊기고 토글이 꺼진 것처럼 보였다 (2026-09-24).
// rotate 가 실패해도 앱을 다음에 열 때 lib/push.js 의 getPushToggleState 가 새 endpoint 를 등록한다.
  ...
  event.waitUntil(
    (async () => {
      ...
      try {
        ...
        await fetch('/api/auth/push/rotate', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ oldEndpoint, ...next.toJSON() }),
        });
      } catch {
        // 재구독·교체 실패 시에는 다음 앱 실행 때 lib/push.js 가 다시 맞춘다
      }
    })()
  );
});
