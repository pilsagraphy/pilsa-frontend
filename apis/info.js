...
export const toggleInfoPostLike = async (postId) => {
  const response = await authApi.patch(`/api/stu/info/posts/${postId}/like`);
  return response.data;
};
...
export const createInfoPost = async ({ title, content, categoryId, files = [] }) => {
  ...
  const response = await authApi.post('/api/stu/info/posts', formData, {
    headers: {
...
export const updateInfoComment = async (postId, commentId, { content, isPrivate = false }) => {
  const response = await authApi.put(`/api/stu/info/posts/${postId}/comments/${commentId}`, {
    content,
    isPrivate: Boolean(isPrivate),
  });
  return response.data;
...
export const deleteInfoComment = async (postId, commentId) => {
  const response = await authApi.delete(`/api/stu/info/posts/${postId}/comments/${commentId}`);
  return response.data;
};
