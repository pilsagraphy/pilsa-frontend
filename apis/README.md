...
7. **신고 관리 - 조치 모달의 '신고자 목록'** — 목록 API 는 대상 단위로 그룹핑해 대표 사유와
...
8. **신고 접수 시 자동 블라인드 미적용** — 기획은 '신고가 들어오면 그 글이 사용자 게시판에서
   ...
   ★상태 필터도 `blind` · `deleted` 만 인식한다 — normal·rejected·restored·resolved 를 보내면
...
9. **신고 관리 - 복원한 신고의 목록 노출** — 기획은 '복원해도 목록에 「복원」으로 남는다'인데
   ...
   기획대로 남기기로 했으므로 서버의 제외 로직 제거와 '복원' 필터 값 추가가 필요하다. 협의 대기.
