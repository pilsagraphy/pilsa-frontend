...
export const verifyFindIdCode = async (email, code) => {
  const response = await authApi.post('/api/auth/id/verify', { email, code });
  return response.data;
};

// 5-2. 아이디 찾기 (GET /api/auth/id/find)
export const findLoginIdByEmail = async (email) => {
  const response = await authApi.get('/api/auth/id/find', {
    params: { email },
  });
...
export const sendPasswordResetVerification = async (loginId, email) => {
  const response = await authApi.get('/api/auth/verification', {
...
export const resetPassword = async ({ loginId, newPassword }) => {
  const response = await authApi.put('/api/auth/password/reset', { loginId, newPassword });
  return response.data;
};
